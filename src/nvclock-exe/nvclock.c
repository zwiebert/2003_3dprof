...
int usage() 
{
  ...
  printf("   -u  --remove-driver\t\tUninstall demand loaded driver (Win32NT)\n\n");
  return 0;
}
...
int main(int argc, char *argv[])
{
...
#if 1
  { // XXX: could be done by environment variable too
    extern int backend_debug;
    int i;
    for (i=1; i < argc; ++i)
  ...
  while ( ( opt = getopt_long (argc, argv, "m:M:n:c:1lfidrsuSh", long_options, NULL)) != -1 )
  {
    switch (opt)
    {
    case 'c':
    ...
    case 'u':
      {
        extern int backend_remove_driver;
        backend_remove_driver = 1;
        break;
      }
