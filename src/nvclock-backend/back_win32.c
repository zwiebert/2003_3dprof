...
#define NO_WINIO   "etc/winio.sys driver installation failed"
#define HAVE_WINIO "\"\\\\.\\NvcWio\" driver succesfully loaded"
...
{
  int dev = 0, dum = 0,  i = -1;
  unsigned short devbusfn = 1;

  card_idx++;
  card[card_idx].device_id = get_device_id();
  card[card_idx].number = 0;
  card[card_idx].card_name = (char*)get_card_name(card[card_idx].device_id, &card[card_idx].gpu);
  card[card_idx].bus_id = (char*)malloc(8 * sizeof(char));
  sprintf(card[card_idx].bus_id, "%02d:%02d.%d", devbusfn >> 8, (devbusfn & 0xff) >> 3, (devbusfn & 0xff) & 0x7);
  card[card_idx].reg_address = get_mem_base(0);

  set_default_speeds(card_idx);
...
#if 0
#include "WinIO.h"
static void debug_list_pci_devices() {
  ...
  while (winio_get_pci_header(winio_instance, &buf)) 
  {
    ...
    if(buf.VendorID == 0x10de)
    {
      ...
      */
      ...
      card_idx++;
      card[card_idx].device_id = buf.DeviceID;
      card[card_idx].number = card_idx;
      card[card_idx].card_name = (char*)get_card_name(card[card_idx].device_id, &card[card_idx].gpu);
      card[card_idx].bus_id = (char*)malloc(8 * sizeof(char));
      sprintf(card[card_idx].bus_id, "%02d:%02d.%d", buf.Bus, PCI_HDR_GET_DEV(&buf), PCI_HDR_GET_FN(&buf));
      card[card_idx].reg_address = buf.BaseAddresses[0];
    }
  }
