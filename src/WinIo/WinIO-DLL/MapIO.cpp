...
PVOID _stdcall MapIO(WinIO vp_inst, ULONG phys_addr, ULONG size)
{
  ...
  if (IsNT)
  {
    ...
    if (!DeviceIoControl(hDriver,IOCTL_WINIO_MAPIO,
      &h->data, sizeof h->data,
      &h->data, sizeof h->data,
      ...
      return (h->usr_ptr = h->data.virt_uaddr);
    ...
    if (!(h->data.virt_uaddr = MapPhysToLin((PBYTE)h->data.phys_addr_requested,
      h->data.size_requested, (HANDLE *)&h->data.virt_kaddr))) {
        handle_free(inst, h);
        return NULL;
      }
...
VOID _stdcall UnMapIO(WinIO vp_inst, PVOID virt_uaddr)
{
  ...
  if (IsNT)
  {
    ...
    /* UnmapPhysicalMemory() does nothing on Win9x, so the next line
    has no effect */
    UnmapPhysicalMemory((HANDLE)h->data.virt_kaddr, (PBYTE)h->data.virt_uaddr);
    handle_free(inst, h);
  }
}
