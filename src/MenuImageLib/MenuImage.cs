  ...
  public class MenuImage : Component, IExtenderProvider {
    ...
    private class MenuHelper {
      ...
      public void DrawMenu (Rectangle bounds, bool selected, int indexValue) {
        ...
        if (_menuItem.Checked || (indexValue == -1 && _menuItem.RadioCheck))
          DrawCheckBox (bounds);
        else {
      ...
      private void DrawCheckBox(Rectangle bounds) {
        ...
        bool radio = _menuItem.RadioCheck;
        bool check = _menuItem.Checked;
        bool enabled = _menuItem.Enabled;

              
        ButtonState btnState = (radio ? 0 : ButtonState.Flat);
        if (radio)                
          btnState |= (check ? ButtonState.Checked : ButtonState.Normal);
        else
          btnState |= (check ? ButtonState.Checked : ButtonState.Flat);
